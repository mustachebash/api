...
promosRouter
	...
	.post('/', authorizeUser, requiresPermission('admin'), async ctx => {
		try {
			const promo = await createPromo({createdBy: req.user.username, ...ctx.request.body});
...
promosRouter
	...
	.delete('/:id', authorizeUser, requiresPermission('admin'), async ctx => {
		try {
			const promo = await updatePromo(ctx.params.id, {updatedBy: req.user.username, status: 'disabled'});
