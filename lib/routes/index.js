...
apiRouter
	.get('/mytickets/pdf', ctx => {
		try {
			...
			ctx.set('Content-disposition', 'attachment; filename="Mustache Bash 2022 Tickets.pdf"');
...
apiRouter
	.post('/check-ins', authorizeUser, requiresPermission('doorman'), async ctx => {
		...
		try {
			...
			if(e.code === 'GUEST_ALREADY_CHECKED_IN') ctx.throw(409, e.context);
			if(e.code === 'EVENT_NOT_ACTIVE') ctx.throw(410, e.context);
			if(e.code === 'EVENT_NOT_STARTED') ctx.throw(412, e.context);
			if(['TICKET_NOT_ACTIVE', 'GUEST_NOT_ACTIVE'].includes(e.code)) ctx.throw(423, e.context);
