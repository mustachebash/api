		...
		const query = r.table('sites')
			...
			.merge({
				events: r.table('events')
					.filter(row => row('site').eq(id)
						.and(row('salesOn'))
						// This allows for automated start times (NOTE: this properly checks the stored timezone against UTC)
						.and(row('openingSales').default(r.time(2000, 1, 1, 'Z')).lt(r.now()))
					)
					.pluck('id', 'salesOn', 'name', 'date', 'currentTicket', 'openingSales')
					.coerceTo('array')
			})
			.merge({
				products: r.table('products')
					...
					.pluck('description', 'id', 'name', 'price', 'status', 'eventId', 'vip', 'quantity', 'sold')
					.coerceTo('array')
			});
