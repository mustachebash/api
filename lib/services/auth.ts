...
export function validateAccessToken(accessToken: string) {
	...
	if (typeof tokenPayload !== 'object' || typeof tokenPayload.sub !== 'string' || !tokenPayload.sub || typeof tokenPayload.role !== 'string')
...
function validateRefreshToken(refreshToken: string) {
	...
	if (typeof tokenPayload !== 'object' || typeof tokenPayload.sub !== 'string' || !tokenPayload.sub || typeof tokenPayload.jti !== 'string' || !tokenPayload.jti)
		throw new AuthServiceError('Invalid token', 'UNAUTHORIZED');
...
export async function authenticateGoogleUser(token: string, { userAgent, ip }: { userAgent: string; ip: string }) {
	...
	try {
		...
		if (e instanceof Error) throw new AuthServiceError(e.message, 'UNAUTHORIZED');
...
export function checkScope(userRole: string, scopeRequired: string) {
	const roles = ['root', 'god', 'admin', 'write', 'read', 'doorman'];
