...
module.exports = {
	async createPromo({ price, type, productId, quantity = 1, recipient, createdBy, email }) {
		...
		const promo = {
			...
			updated: r.now(),
			status: 'active',
			// How many tickets does this promo allow?
			quantity,
			createdBy,
			productId,
			type
	...
	getPromos({ eventId } = {}) {
		let query = r.table('promos');

		if(eventId) query = query.getAll(r.args(
			r.table('products').filter({eventId})('id').coerceTo('array')
		), {index: 'productId'});
