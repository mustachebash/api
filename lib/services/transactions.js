...
module.exports = {
	...
	async createTransaction({ paymentMethodNonce, cart = [], customer = {} }) {
		...
		const response = await gateway.transaction.sale({
			...
			amount: orderDetails.map(i => Number(i.quantity) * i.product.price).reduce((tot, cur) => tot + cur, 0),
		...
		orderDetails.forEach(i => {
			if(i.product.type === 'ticket') {
				newGuests = newGuests.concat(i.data.guests.map(g => ({
					...g,
		...
		// Don't let this fail the transaction
		try {
			await run(r.table('transactions').insert(transaction));
		} catch(e) {
			console.error(e);
		}
