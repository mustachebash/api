	...
	async createTransaction({ paymentMethodNonce, cart = [], customer = {}, promoId, donation }) {
		...
		const orderDetails = cart.map(i => {
			const product = products.find(p => p.id === i.productId),
				remaining = product.quantity ? product.quantity - (product.sold || 0) : null;

			if(remaining !== null) {
				if(remaining < i.quantity) throw new TransactionsServiceError('Purchase exceeds remaining quantity', 'INVALID');

				const totalSold = (product.sold || 0) + i.quantity;
				productsToUpdate.push({
					id: product.id,
					sold: totalSold,
					// Mark as archived if this transaction sells the item out
		...
		// If product sales need to update, do so now
		// NOTE: THis is an absolutely awful hacky way to enforce individual quantity limits (vs. total event capacity),
		// but the alternative of calculating a live sales total is not feasible until order items are their own table
		productsToUpdate.forEach(async p => {
			try {
				await run(r.table('products').get(p.id).update({
					sold: p.sold,
					updated: r.now(),
					status: p.status
				}));
			} catch(e) {
				log.error(e, 'Error updating product sales totals');
			}
		});
