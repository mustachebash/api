...
module.exports = {
	...
	async createTransaction({ paymentMethodNonce, cart = [], customer = {}, promoId, donation }) {
		...
		if(promo && promo.price) {
			amount = promo.price + serviceFee + promoDonationAmount;
		}
	...
	async getTransactionProcessorDetails(id) {
		...
		try {
			return await gateway.transaction.find(transaction.braintreeTransactionId);
		} catch(e) {
	...
	async refundTransaction(id, username) {
		...
		try {
			...
			const updated = r.now();
			await Promise.all([
				run(r.table('transactions').get(id).update({status: 'refunded', updatedBy: username, updated})),
