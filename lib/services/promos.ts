...
export async function getPromos({ eventId }: GetPromosQuery = {}): Promise<Promo[]> {
	try {
		let promos: Promo[];
		if(eventId) {
...
export async function updatePromo(id: string, updates: UpdatePromoInput): Promise<Promo> {
