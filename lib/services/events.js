...
module.exports = {
		...
		try {
			const [summary] = (await sql`
				SELECT
					...
					count(g.id) FILTER (WHERE g.created::date = now()::date) as guests_today,
	...
	async getEventExtendedStats(id) {
		try {
			const [extendedStats] = (await sql`
				...
				SELECT
					...
					) FILTER (WHERE NOT pa.promo) as sales_tiers,
	...
	getEventChart() {
		throw new EventsServiceError('Not yet implemented', 'NOT_IMPLEMENTED');
