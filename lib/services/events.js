	...
	getEventSummary(id) {
		const query = r.table('products').getAll(id, {index: 'eventId'}).coerceTo('array').do(eventProducts => {
			return r.table('events')
				...
				.merge(e => ({
					eventId: e('id'),
					totalGuests: r.table('guests').getAll(e('id'), {index: 'eventId'}).filter(row => row('status').eq('active')).count(),
					totalCompedGuests: r.table('guests').getAll(e('id'), {index: 'eventId'}).filter(row => row('confirmationId').eq('COMPED')).count(),
					totalRevenue: r.table('transactions')
						.filter(row => row.hasFields('promoId').not())
						.filter(row => row('status').default('default').ne('refunded').and(row('status').default('default').ne('voided')))
