...
type GuestUpdate = Partial<Pick<Guest, 'status' | 'firstName' | 'lastName' | 'meta' | 'admissionTier'>> & {
	updatedBy?: string;
	checkInTime?: Date | null | unknown;
};
...
export async function getGuests({ limit, eventId, admissionTier, createdReason, orderBy = 'created', sort = 'desc' }: GuestFilters): Promise<Guest[]> {
	try {
